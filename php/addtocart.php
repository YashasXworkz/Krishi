...
if (isset($_POST["add"])) {
    ...
    $sql3 = mysqli_query(
        $conn,
        "SELECT * FROM cart WHERE userid='$cid' and farmerid='$fid' and prodid='$pid' "
    );
    ...
    if ($row) {
        ...
        $quantity = 1;
        $sql1 = "INSERT INTO cart (userid, prodid, farmerid, cart_quantity, flag) VALUES('$cid', '$pid', '$fid', '$quantity', 1)";
        mysqli_query($conn, $sql1);
        if (mysqli_error($conn)) {
            echo "Errorcode: " . mysqli_errno($conn);
        }
